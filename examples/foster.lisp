...
(setf *r-foster* 
                  (cm:new cm:weighting
                    :of `((,(cm:new cm:cycle :of '(h h)) :weight .375)
                          (,(cm:new cm:cycle :of '(q q q q)) :weight .125)
                          (,(cm:new cm:cycle :of '(h q q)) :weight .125)
                          (,(cm:new cm:cycle :of '(q q h)) :weight .125)
                          (,(cm:new cm:cycle :of '(q h q)) :weight .25)
                          (w :weight .125))))

(defun foster (time &optional notes rhythms root chan)
  (let ((rhythm (cm:rhythm (cm:next rhythms) 120)))
    (play-midi-note
     time (cm:keynum (cm:transpose (cm:next notes) root)) 30 rhythm chan)
    (aat (+ time #[rhythm b])
         #'foster
         it notes rhythms root chan)))

(foster (tempo-sync #[1 b]) *m-foster* *r-foster* -12 0)
(foster (tempo-sync #[1 b]) *m-foster* *r-foster*   0 1)
(foster (tempo-sync #[1 b]) *m-foster* *r-foster*  12 2)
(foster (tempo-sync #[1 b]) *m-foster* *r-foster*  24 3)
