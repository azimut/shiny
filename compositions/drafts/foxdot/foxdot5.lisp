...
(fx-load-simple "/home/sendai/.local/lib64/python3.4/site-packages/FoxDot/snd/o/lower/1_snare_drum.wav" "o")

(setf (bpm *tempo*) 160d0)

;; d1 >> play(P["V::"][:16] & P["<v ><  |o1| >"], drive=0.1, rate=1.2)
(destructuring-bind (p1 p2 p3) (cons (cm:new cm:cycle :of (repeat 16 '(V _ _)))
                                     (fx-pat "<v ><  o >"))
  (defun f (time)
    (bbplay (next p1) :amp .1 :rate 1.2)
    (bbplay (next p2) :amp .5 :rate 1.2)
    (bbplay (next p3) :amp .5 :rate 1.2)
    (aat (+ time #[1 b]) #'f it)))
...
(let ((dur (cm:new cm:cycle :of (pdur 3 8)))
